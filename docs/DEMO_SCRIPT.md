# Reprieve — 3-Minute Demo Video Script

**Target Time:** 2 minutes 45 seconds (Hard limit: 3 minutes)  
**Tone:** Calm, authoritative, executive fintech & banking  

---

### Scene 1: The Human Problem & Pre-Default Detection (0:00 - 0:30)
- **Visual:** Open `http://localhost:3000/`. Show the Portfolio Radar tab with 40 accounts and the Left Phone Simulator.
- **Narrator:**
  ...
  > *Traditional lending systems reach borrowers after the payment bounces, using aggressive recovery calls and penal fees. Reprieve takes a hardship-first approach: deterministic cash-flow signals flag Meera's stress before default, and reaches out proactively with workable options."*

---

### Scene 2: Autonomous Relief & Cedar Policy Envelope (0:30 - 1:05)
- **Visual:** In the phone chat, Meera asks: *"Can I get a 7-day shift?"* Reprieve proposes the plan card. Click **Policy Envelope & Tools** tab to show the green gauge zone and Cedar code. Meera presses **Accept Relief Plan**.
- **Narrator:**
...
- **Visual:** Click Hero button **Arjun (30d Review)**. Arjun asks for 30 days due to festival slowdown. Switch to **Step Functions Approvals** tab. Point to the amber waiting state. Click **Approve Concession**.
- **Narrator:**
...
- **Visual:** Switch to **Decision Log & Tamper Demo** tab. Click **Simulate DB Tampering** (turns crimson). Click **Restore from S3 Lock** (turns emerald). Click **Verify Chain Integrity**.
- **Narrator:**
...
- **Visual:** Switch to **A/B Impact & Unit Economics** tab, then **AWS Cloud Topology** tab.
- **Narrator:**
  > *"In simulated clinical trials across Treated vs Control cohorts, Reprieve achieves a +27.4 percentage point cure uplift. Running serverless with Amazon Bedrock Nova Lite, each borrower intervention costs under $0.0031.*  
  > *Reprieve: Relief within limits, proven cryptographically."*
